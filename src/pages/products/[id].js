...
export default function ProductPage({product}) {
    ...
    function onAddToCart(event) {
        ...
        toast.success(`${count} ${product.name} added successfully`, {id})
     }
                ...
                <div className="w-full flex-1 max-w-md border border-opacity-50 rounded-md shadow-lg p-6 bg-white">
                    ...
                    <div className="mt-4 border-t pt-4">
                        ...
                        <div className="mt-1 flex items-center space-x-3">
                            <button
                            // Disable minus button if quantity is less than 1
                             disabled={count <= 1}
                    ...
                    <button
                     onClick={onAddToCart}
                     className="w-full rounded-md mt-4 border border-lime-500 py-2 px-6 bg-lime-500 hover:bg-lime-600 hover:border-lime-600 focus:ring-4 focus:ring-opacity-50 focus:ring-line-500 text-white disabled:opacity-50 disabled:cursor-not-allowed">
                        Add to cart
                    </button>
                </div>
            </div>
    ...
    const inventory = await stripe.products.list({
      ...
      return {
        props: {
          ...
          // Revalidate data every hour if new product is added
          revalidate: 60 * 60,
      }
}